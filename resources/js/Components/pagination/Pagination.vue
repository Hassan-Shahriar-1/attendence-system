<template>
	<div class="mt-[20px]">
		<div class="flex items-center flex-wrap justify-between">
			...
			<div class="flex items-center flex-nowrap">
				<div class="ml-2 flex items-center gap-2 text-xs">
					<PageDropdown
						...
						"
					/>
					<div class="text-xs text-[#565c67] font-medium leading-[21px]">
						Entries per page
					</div>
				</div>
			</div>
			...
			<!-- Page Links starts -->
			<div class="flex flex-nowrap gap-2" v-if="paginationData">
				...
				>
					<template #prev-button>
						<div class="flex items-center">
							<ArrowLeftIcon />
							<div class="text-center text-[#565c67] text-sm font-medium leading-tight">
								Previous
							</div>
						</div>
					</template>
...
:deep(.paginate-buttons) {
	@apply text-[#585754] w-auto h-8 px-4 py-3 rounded-lg active:bg-[#f9f9f9] hover:bg-[#f9f9f9] hover:border-b hover:border-primary hover:text-primary flex-col justify-center items-center inline-flex cursor-pointer text-center text-base font-normal leading-snug;
}
:deep(.active-page) {
	@apply bg-[#f9f9f9] text-word-semibold text-primary font-bold border-b border-primary;
}
:deep(.active-page:hover) {
	@apply bg-[#f9f9f9] font-bold;
}
</style>
